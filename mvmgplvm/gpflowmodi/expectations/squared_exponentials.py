...
from ..probability_distributions import DiagonalGaussian, Gaussian, MarkovGaussian, DiagonalMixtureGaussian,MixtureGaussian
...
#VMGPLVM
@dispatch.expectation.register(MixtureGaussian, kernels.SquaredExponential, NoneType, NoneType, NoneType)
def _E(p, kernel, _, __, ___, nghp=None):
    """
    Compute the expectation:
    <diag(K_{X, X})>_p(X)
...
#VMGPLVM
@dispatch.expectation.register(
    MixtureGaussian, kernels.SquaredExponential, InducingPoints, NoneType, NoneType
)
def _E(p, kernel, inducing_variable, _, __, nghp=None):
    """
    Compute the expectation:
    ...
    chol_L_plus_Xcov = tf.linalg.cholesky(tf.linalg.diag(lengthscales ** 2) + Xcov)  # KxNxDxD

    all_diffs = tf.transpose(Z) - tf.expand_dims(Xmu, 3)  # KxNxDxM

    exponent_mahalanobis = tf.linalg.triangular_solve(
        chol_L_plus_Xcov, all_diffs, lower=True
    )  # KxNxDxM
    
    exponent_mahalanobis = tf.reduce_sum(tf.square(exponent_mahalanobis), 2)  # KxNxM
    exponent_mahalanobis = tf.exp(-0.5 * exponent_mahalanobis)  # KxNxM

    sqrt_det_L = tf.reduce_prod(lengthscales)
    sqrt_det_L_plus_Xcov = tf.exp(
        tf.reduce_sum(tf.math.log(tf.linalg.diag_part(chol_L_plus_Xcov)), axis=2)
    )
    determinants = sqrt_det_L / sqrt_det_L_plus_Xcov  # KxN
...
def _E(p, kern1, feat1, kern2, feat2, nghp=None):
    ...
    # use only active dimensions
    Xcov = kernel.slice_cov(tf.linalg.diag(p.cov) if isinstance(p, DiagonalMixtureGaussian) else p.cov)
    Z, Xmu = kernel.slice(inducing_variable.Z, p.mu)
    ...
    sqrt_det_L = tf.reduce_prod(0.5 * squared_lengthscales) ** 0.5
    C = tf.linalg.cholesky(0.5 * tf.linalg.diag(squared_lengthscales) + Xcov)  # KxNxDxD
    dets = sqrt_det_L / tf.exp(tf.reduce_sum(tf.math.log(tf.linalg.diag_part(C)), axis=2))  # KxN
    
    C_inv_mu = tf.linalg.triangular_solve(C, tf.expand_dims(Xmu, 3), lower=True)  # KxNxDx1
    
    C_inv_z = tf.linalg.triangular_solve(
        C, tf.tile(tf.expand_dims(tf.expand_dims(0.5 * tf.transpose(Z), 0),0), [K, N, 1, 1]), lower=True
    ) # KxNxDxM
    
    mu_CC_inv_mu = tf.expand_dims(tf.reduce_sum(tf.square(C_inv_mu), 2), 3)  # KxNx1x1
    z_CC_inv_z = tf.reduce_sum(tf.square(C_inv_z), 2)  # KxNxM
    zm_CC_inv_zn = tf.linalg.matmul(C_inv_z, C_inv_z, transpose_a=True)  # KxNxMxM
    two_z_CC_inv_mu = 2 * tf.linalg.matmul(C_inv_z, C_inv_mu, transpose_a=True)[:, :, :, 0]  # KxNxM
    # KxNxMxM
    exponent_mahalanobis = (
        mu_CC_inv_mu
        + tf.expand_dims(z_CC_inv_z, 2)
        + tf.expand_dims(z_CC_inv_z, 3)
        + 2 * zm_CC_inv_zn
        - tf.expand_dims(two_z_CC_inv_mu, 3)
        - tf.expand_dims(two_z_CC_inv_mu, 2)
    )
    exponent_mahalanobis = tf.exp(-0.5 * exponent_mahalanobis)  # KxNxMxM
    
    # Compute sqrt(self(Z)) explicitly to prevent automatic gradient from
    # being NaN sometimes, see pull request #615
    kernel_sqrt = tf.exp(-0.25 * square_distance(Z / kernel.lengthscales, None))
    return kernel.variance ** 2 * kernel_sqrt * tf.reshape(dets, [K, N, 1, 1]) * exponent_mahalanobis
